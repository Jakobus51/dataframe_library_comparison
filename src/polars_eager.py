...
class PolarsEager:
    ...
    def load_data(self):
        ...
        customer_database = pl.read_parquet(self.save_location / f"customer_database.parquet")
        ...
        store_visits = pl.read_parquet(self.save_location / f"store_visits.parquet")
        ...
        advertisements = pl.read_parquet(self.save_location / f"advertisements.parquet")
        ...
        revenue = pl.read_parquet(self.save_location / f"revenue.parquet")
    ...
    def data_exploration(self):
        ...
        num_nans = self.customer_database.select(pl.col("brand_happiness").is_nan().sum()).to_numpy()[0][0]
    ...
    def _get_final_table(self) -> pl.DataFrame:
        """Create the final table"""
        # Aggregate per channel and pivot so that each channel is a column
        daily_advertisements = self.advertisements.group_by("datetime", "channel").agg(
            pl.col("spend").sum().alias("spend"),
        )
        daily_advertisements = daily_advertisements.pivot(index="datetime", values="spend", columns="channel")
